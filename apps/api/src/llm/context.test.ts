...
function makeEntry(id: string, title: string, summary: string, tags: string[], updated = '2026-01-01T00:00:00Z'): IndexEntry {
  return { id, type: 'idea', title, status: 'active', tags, summary, created: updated, updated, path: `ideas/${id}.md` }
}
...
describe('assembleContext', () => {
  ...
  it('maps to ContextEntry shape (no extra fields)', () => {
    ...
    expect(Object.keys(result).sort()).toEqual(['id', 'session_summary', 'status', 'summary', 'tags', 'title', 'type'].sort())
  })
})
...
describe('formatContextBlock', () => {
  ...
  it('formats entries with type, title, status, summary, tags', () => {
    const block = formatContextBlock([{ id: '1', type: 'idea', title: 'My Idea', status: 'active', tags: ['a', 'b'], summary: 'Good idea' }])
  ...
  it('includes session_summary when present', () => {
    const block = formatContextBlock([{ id: '1', type: 'plan', title: 'T', status: 'draft', tags: [], summary: 's', session_summary: 'Last time we discussed X' }])
    expect(block).toContain('Last session: Last time we discussed X')
  })

  it('omits Last session line when session_summary absent', () => {
    const block = formatContextBlock([{ id: '1', type: 'idea', title: 'T', status: 'draft', tags: [], summary: 's' }])
