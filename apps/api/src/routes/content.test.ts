...
const ENTRIES: IndexEntry[] = [
  { id: 'idea-1', type: 'idea', title: 'TypeScript Idea', status: 'active', tags: ['ts'], summary: 'An idea', created: '2026-01-01T00:00:00Z', updated: '2026-01-02T00:00:00Z', path: 'ideas/ts-idea.md' },
  { id: 'plan-1', type: 'plan', title: 'My Plan', status: 'parked', tags: ['planning'], summary: 'A plan', created: '2026-01-01T00:00:00Z', updated: '2026-01-01T00:00:00Z', path: 'plans/my-plan.md' },
]

const mockCache = {
  all: vi.fn().mockReturnValue(ENTRIES),
  search: vi.fn((q: string) => ENTRIES.filter(e => e.title.toLowerCase().includes(q) || e.summary.toLowerCase().includes(q))),
  findById: vi.fn((id: string) => ENTRIES.find(e => e.id === id)),
} as unknown as IndexCache

const mockGithub = {
  listOpenPRs: vi.fn().mockResolvedValue([
    { number: 42, title: 'Draft Idea', head: { ref: 'idea/abc-draft' } },
  ]),
