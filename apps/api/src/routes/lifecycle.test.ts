...
const entryWithPR = { id: 'idea-1', type: 'idea', title: 'T', status: 'draft', tags: [], summary: 's', created: '', updated: '', path: 'ideas/t.md', pr: 7, branch: 'idea/abc' }
...
describe('POST /api/capture', () => {
  it('returns 401 without auth', async () => {
    const res = await app.request('/api/capture', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({}) })
    expect(res.status).toBe(401)
  })

  it('creates branch, file, and PR then returns ids', async () => {
    const res = await post('/api/capture', { type: 'idea', title: 'New Idea', tags: ['ts'], summary: 's', body: 'body text' })
